...
# Build the firmware (release mode recommended for embedded)
cargo build --release

# The UF2 file will be at: target/thumbv6m-none-eabi/release/productiondeck.uf2
```

### Build Output
After successful build, you'll find these files in `target/thumbv6m-none-eabi/release/`:
- `productiondeck` - ELF executable with debug symbols  
- `productiondeck.uf2` - Main firmware file for flashing (generated by elf2uf2-rs)

The UF2 file is automatically generated thanks to the runner configuration in `.cargo/config.toml`.
