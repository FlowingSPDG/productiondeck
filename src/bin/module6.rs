//! ProductionDeck - Stream Deck Module 6 Keys Compatible Firmware
//! 
//! This binary builds firmware specifically for Stream Deck Module 6 Keys compatibility:
//! - 6 keys in 3x2 layout
//! - 80x80 pixel images per key
//! - USB VID:PID 0x0fd9:0x00b8
//! - Module protocol (65-byte input, 1024-byte output, 32-byte feature reports)
...
const DEVICE: productiondeck::device::Device = productiondeck::device::Device::Module6;
...
/// Main application entry point for Stream Deck Module 6 Keys with multicore support
#[cortex_m_rt::entry]
fn main() -> ! {
    // Initialize hardware
    let p = embassy_rp::init(Default::default());
    
    // Create application supervisor for Module 6Keys
    ...
    executor0.run(|spawner| {
        ...
        // Spawn button task for Module 6Keys (Direct mode)
...
async fn core0_main_task(mut supervisor: supervisor::AppSupervisor) {
    ...
    info!("Core 0: Stream Deck Module 6 Keys firmware initialized successfully");
