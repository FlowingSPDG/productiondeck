//! ProductionDeck - Stream Deck Module 15 Keys Compatible Firmware
//! 
//! This binary builds firmware specifically for Stream Deck Module 15 Keys compatibility:
...
const DEVICE: productiondeck::device::Device = productiondeck::device::Device::Module15;
...
/// Main application entry point for Stream Deck Module 15 Keys with multicore support
#[cortex_m_rt::entry]
fn main() -> ! {
    // Initialize hardware
    let p = embassy_rp::init(Default::default());
    
    // Create application supervisor for Module 15 Keys
    ...
    executor0.run(|spawner| {
        ...
        // Spawn button task for Module 15 Keys (Direct mode for now)
        unwrap!(spawner.spawn(buttons::button_task_direct({
            let mut inputs = heapless::Vec::new();
            // 15 keys: 15 individual inputs
...
async fn core0_main_task(mut supervisor: supervisor::AppSupervisor) {
    ...
    info!("Core 0: Stream Deck Module 15 Keys firmware initialized successfully");
