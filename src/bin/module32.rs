//! ProductionDeck - Stream Deck Module 32 Keys Compatible Firmware
//! 
//! This binary builds firmware specifically for Stream Deck Module 32 Keys compatibility:
//! - 32 keys in 8x4 layout
//! - 96x96 pixel images per key
//! - USB VID:PID 0x0fd9:0x00ba
//! - Module protocol with 180° image rotation
...
const DEVICE: productiondeck::device::Device = productiondeck::device::Device::Module32;
...
/// Main application entry point for Stream Deck Module 32 Keys with multicore support
#[cortex_m_rt::entry]
fn main() -> ! {
    // Initialize hardware
    let p = embassy_rp::init(Default::default());
    
    // Create application supervisor for Module 32 Keys
    ...
    executor0.run(|spawner| {
        ...
        // Spawn button task for Module 32 Keys (Direct mode for now)
        // Note: RP2040 has limited GPIO pins, using 22 pins for 32 keys (matrix mode would be better)
        unwrap!(spawner.spawn(buttons::button_task_direct({
            let mut inputs = heapless::Vec::new();
            // 22 keys: using available GPIO pins (GP2-GP29, excluding used pins)
            let _ = inputs.push(embassy_rp::gpio::Input::new(p.PIN_2, embassy_rp::gpio::Pull::Up));
            let _ = inputs.push(embassy_rp::gpio::Input::new(p.PIN_3, embassy_rp::gpio::Pull::Up));
            ...
            let _ = inputs.push(embassy_rp::gpio::Input::new(p.PIN_7, embassy_rp::gpio::Pull::Up));
            let _ = inputs.push(embassy_rp::gpio::Input::new(p.PIN_8, embassy_rp::gpio::Pull::Up));
            let _ = inputs.push(embassy_rp::gpio::Input::new(p.PIN_9, embassy_rp::gpio::Pull::Up));
            ...
            let _ = inputs.push(embassy_rp::gpio::Input::new(p.PIN_13, embassy_rp::gpio::Pull::Up));
            let _ = inputs.push(embassy_rp::gpio::Input::new(p.PIN_14, embassy_rp::gpio::Pull::Up));
            let _ = inputs.push(embassy_rp::gpio::Input::new(p.PIN_15, embassy_rp::gpio::Pull::Up));
            let _ = inputs.push(embassy_rp::gpio::Input::new(p.PIN_16, embassy_rp::gpio::Pull::Up));
            let _ = inputs.push(embassy_rp::gpio::Input::new(p.PIN_17, embassy_rp::gpio::Pull::Up));
            let _ = inputs.push(embassy_rp::gpio::Input::new(p.PIN_18, embassy_rp::gpio::Pull::Up));
            let _ = inputs.push(embassy_rp::gpio::Input::new(p.PIN_19, embassy_rp::gpio::Pull::Up));
            let _ = inputs.push(embassy_rp::gpio::Input::new(p.PIN_22, embassy_rp::gpio::Pull::Up));
            let _ = inputs.push(embassy_rp::gpio::Input::new(p.PIN_23, embassy_rp::gpio::Pull::Up));
            let _ = inputs.push(embassy_rp::gpio::Input::new(p.PIN_24, embassy_rp::gpio::Pull::Up));
            let _ = inputs.push(embassy_rp::gpio::Input::new(p.PIN_29, embassy_rp::gpio::Pull::Up));
            inputs
        })));
...
async fn core0_main_task(mut supervisor: supervisor::AppSupervisor) {
    ...
    info!("Core 0: Stream Deck Module 32 Keys firmware initialized successfully");
        ...
        match receiver.receive().await {
            productiondeck::types::DisplayCommand::DisplayImage { key_id, data } => {
                ...
                if data.len() <= image_processing_buffer.len() {
                    ...
                    // TODO: Implement actual image processing and display with 180° rotation
