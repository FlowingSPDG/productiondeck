...
/// Main application entry point for StreamDeck Mini
...
async fn main(spawner: Spawner) {
    ...
    // Initialize and spawn all hardware tasks for Mini  
    match hardware::init_hardware_tasks_for_device(&spawner, p, DEVICE).await {
        Ok(()) => {
            info!("StreamDeck Mini firmware initialized successfully");
