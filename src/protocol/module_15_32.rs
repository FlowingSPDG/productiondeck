...
impl Module15_32KeysHandler {
    pub fn new() -> Self { Self { model: ModuleModel::Module15 } }
    pub fn with_model(model: ModuleModel) -> Self { Self { model } }
...
impl ProtocolHandlerTrait for Module15_32KeysHandler {
    fn version(&self) -> ProtocolVersion { ProtocolVersion::Module15_32Keys }
    ...
    fn map_buttons(&self, physical_buttons: &[bool], cols: usize, rows: usize, left_to_right: bool) -> ButtonMapping {
        let max = match self.model { ModuleModel::Module15 => 15, ModuleModel::Module32 => 32 };
    ...
    fn input_report_size(&self, _button_count: usize) -> usize { 512 }
    ...
    fn format_button_report(&self, buttons: &ButtonMapping, report: &mut [u8]) -> usize {
        ...
        let max_keys = match self.model { ModuleModel::Module15 => 15, ModuleModel::Module32 => 32 };
        ...
        if let Some(cmd) = self.parse_module_get_command(report_id) {
            match cmd {
                ModuleGetCommand::GetFirmwareVersion(ftype) => {
                    ...
                    let start = 6; let end = (start + ver.len()).min(total_len);
                ...
                ModuleGetCommand::GetUnitSerialNumber => {
                    ...
                    let start = 2; let end = (start + data_len as usize).min(total_len);
