...
impl AppSupervisor {
    ...
    pub fn print_startup_banner(&self) {
        ...
        info!("USB: VID=0x{:04X} PID=0x{:04X}", usb_config.vid, usb_config.pid);
        info!("Protocol: {:?}", usb_config.protocol);
        info!("Keys: {} ({}x{} layout)", layout.total_keys, layout.cols, layout.rows);
        info!("Display: {}x{} per key", display.image_width, display.image_height);
        info!("========================================");
    }
    ...
    pub fn print_init_success(&self) {
        ...
        info!("USB VID:PID = {:04X}:{:04X}", usb_config.vid, usb_config.pid);
